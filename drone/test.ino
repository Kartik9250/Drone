...
void loop(){
  ...
  //Beep when battery voltage is too low
  if(below_voltage_threshold) {
    
    if(buzzerCounter >= 0 && buzzerCounter < buzzerMax) {
      digitalWrite(buzzerPin, HIGH);
      buzzerCounter++;
    } 

    if(buzzerCounter == buzzerMax) {
      digitalWrite(buzzerPin, LOW);
      buzzerCounter = -1;
    }
  ...
  if(start != 0 && rememberStart != 0 && (micros() - loop_timer > 4050))digitalWrite(12, HIGH);                   //Turn on the LED if the loop time exceeds 4050us.
 

 if(rememberStart == 0 && start != 0) {
